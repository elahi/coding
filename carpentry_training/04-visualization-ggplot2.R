...
ggplot(data = surveys_complete, 
       aes(species_id, hindfoot_length)) + 
  geom_jitter(alpha = 0.3, aes(color = as.character(plot_id))) + 
  geom_boxplot(alpha = 0) 
...
#' 4. Recreate 'surveys_summary', but now add the standard error
#' (google the formula for standard error if you have not learned this or forgotten)

#' 5. Now on the plot, add error bars for the standard deviation
...
# 5. Get standard errors and plot those
surveys_summary <- surveys_complete %>% 
  filter(!is.na(weight)) %>% 
  group_by(species_id) %>% 
  summarise(wt_mean = mean(weight), 
