...
## Specific date
p1 <- readGPX("GPX/Waypoints_10-OCT-19.gpx")
names(p1)
p1
...
df <- p1$waypoints
gps_names <- p1$waypoints[[5]]

plot(lat ~ lon, data = p1$waypoints)
text(x = p1$waypoints$lon, y = p1$waypoints$lat, labels = gps_names)
write.csv(p1$waypoints, 'output/gpx_191010_pinar.csv')
