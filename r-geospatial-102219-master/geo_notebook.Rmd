...
```{r}
setwd("~/github/coding/r-geospatial-102219-master")
```
...
```{r}
ptpn <- raster("./data/PtPn_SB_2m_gs_interp.tif")
ptpn
plot(ptpn)
ptpn100 <- calc(ptpn, function(x) ifelse(x < 100, x, NA))
plot(ptpn100)

ptpn2 <- ptpn - cellStats(ptpn, stat='mean')
plot(ptpn2)
```
...
ggplot() + 
  geom_sf(data = ne_states, aes(fill = ALAND)) + 
  facet_wrap(~ STATENS)
